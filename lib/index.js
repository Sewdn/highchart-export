...
var _exportChart = function (processingDir, hcExportRequest, callback) {
    var outputChartName = hcExportRequest.filename,
        ...
        inputFormat = hcExportRequest.options ? 'json' : 'svg',
        input = hcExportRequest.options ? JSON.stringify(hcExportRequest.options) : hcExportRequest.svg,
        inFile = processingDir + outputChartName + '.' + inputFormat,
        exportInfo = {
    ...
    fs.writeFile(inFile, input, function (svgErr) {
        if (inputFormat === 'svg' && outputFormat == 'svg+xml') {
            callback(null, exportInfo);
        }
        else {
            var args = [exportjs, '-infile', inFile, '-outfile', outputFilePath, '-scale', scale, '-width', width , '-constr', 'Chart'];
            execFile(phantomjsCmd, args, function (err, stdout, stderr) {
                if (err) {
                    callback({message: err}, exportInfo);
                } else if (stdout.length > 0) { // PhantomJS always outputs to stdout.
                    if(stdout.toString().trim()===outputFilePath){
                        callback(null, exportInfo);
                    }else{
                        callback(new Error(stdout.toString().trim()), exportInfo);
                    }
                } else if (stderr.length > 0) { // But hey something else might get to stderr.
                    callback(new Error(stderr.toString().trim()), exportInfo);
                } else {
                    callback(null, exportInfo);
                }
            });
